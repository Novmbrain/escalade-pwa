...
- **建议**: 确保整个卡片区域都可点击，增加 touch feedback (如轻微缩放或 ripple 效果)，让用户知道点击已被识别。
...
- **建议**:
  - 搜索结果增加线路描述的前几个字或标签 (如 "有 Beta 视频" 标记)
  - 空白状态可以展示热门线路或最近添加的线路作为推荐
  - 搜索为空时底部的 "Browse all routes" 很好，但可以更突出
- **相关文件**: `src/components/search-overlay.tsx`

#### 5. 难度选择器可发现性

- **问题**: 难度选择器 (0-13 数字条) 在 Routes 页面，"Tap or drag to select grade range" 提示文字颜色偏淡，新用户可能不知道可以拖拽选范围。
- **建议**:
  - 第一次使用时加个简短 tooltip 动画提示
  - 或者默认高亮一个常见范围 (如 V2-V5) 让用户意识到这是可交互的
