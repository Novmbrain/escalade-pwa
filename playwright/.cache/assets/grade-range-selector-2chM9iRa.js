import { g as getDefaultExportFromCjs, r as reactExports } from './index-XTowZftG.js';
...
const GRADE_GROUPS = V_GRADES.map((grade) => ({
  label: grade,
  value: grade,
  grades: [grade],
  color: getGradeColor(grade)
}));
function getGradesByValue(value) {
  const group = GRADE_GROUPS.find((g) => g.value === value);
  return group ? [...group.grades] : [];
}
function getGradesByValues(values) {
...
function GradeRangeSelector({
  ...
  const [dragEnd, setDragEnd] = reactExports.useState(null);
  const [hasMoved, setHasMoved] = reactExports.useState(false);
  const [optimisticSelection, setOptimisticSelection] = reactExports.useState(null);
  reactExports.useEffect(() => {
    setOptimisticSelection(null);
  }, [selectedGrades]);
  const displayedSelection = optimisticSelection ?? selectedGrades;
  const getGradeIndexFromPosition = reactExports.useCallback((clientX) => {
  ...
  const getSelectedFromRange = reactExports.useCallback((start, end) => {
    const min = Math.min(start, end);
    const max = Math.max(start, end);
    return V_GRADES.slice(min, max + 1);
  }, []);
  const toggleGrade = reactExports.useCallback((index) => {
  ...
  const handleDragStart = reactExports.useCallback((clientX) => {
  ...
  const handleDragMove = reactExports.useCallback((clientX) => {
  ...
  const isGradeSelected = reactExports.useCallback((index) => {
  ...
  const getRangeText = () => {
    ...
    const indices = displayedSelection.map((g) => V_GRADES.indexOf(g)).filter((i) => i >= 0).sort((a, b) => a - b);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mb-2", children: [
      ...
      displayedSelection.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          onClick: handleClear,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        ...
        children: V_GRADES.map((grade, index) => {
          ...
          return /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
              ...
              style: {
                ...
                boxShadow: selected ? "inset 0 0 0 1px rgba(255,255,255,0.3)" : void 0
              },
              children: grade.replace("V", "")
            },
            grade
          );
        })
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "p",
      {
        ...
        children: "点击切换单个难度，拖动选择范围"
      }
    )
  ] });
}

export { GradeRangeSelector };
//# sourceMappingURL=grade-range-selector-2chM9iRa.js.map
